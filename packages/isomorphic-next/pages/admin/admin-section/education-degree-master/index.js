...
const EducationDegreeMaster = () => {
    ...
    const load = async () => {
      ...
      if (!response.data.isEmpty) {
        const dataSource = response.data.map((res, index) => ({
          sr_no: index + 1,
          key: res.education_id,
          education_degree: res.education_degree,
        }));
        setEducations(dataSource);
        setLoading(false);
      }
    };
  ...
  const onConfirmDelete = async (id) => {
    await client.delete(`/user/public/education_degree_delete/${id}/`);
    const data = educations.filter((document) => document.key !== id);
    setEducations(data);
  ...
  return (
    <>
      <Head>
        <title>Education Degree List</title>
      </Head>
      <DashboardLayout>
          ...
          <ManageJobPostStyles>
            {loading ? (
              ...
              <Space direction='vertical' style={{ width: '100%' }}>
                <PageHeader>Education Degree List</PageHeader>
                <Row className='action-bar'>
                  <Col span={24}>
                    <Row justify='end'>
                      <Row span={12}>
                        <Link href='/admin/admin-section/education-degree-master/add'>
                ...
                <ListingStyles>
                  <Row>
                    <Col span={24}>
                      <Table dataSource={educations} getPopupContainer={(trigger) => trigger.parentNode}>
                        ...
                        <Column
                          ...
                          render={(text, record) => (
                            <Space size='middle'>
                              <Link href={`/admin/admin-section/education-degree-master/edit/${record.key}`}>
                                <a href={`/admin/admin-section/education-degree-master/edit/${record.key}`}>
...
export default EducationDegreeMaster;
