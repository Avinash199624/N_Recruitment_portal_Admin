...
const Edit = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const response = await client.get(`/document/document_type/${id}/`);
      setInitialState({
        ...response.data,
  ...
  const onFormSubmit = async (values) => {
    await client.put(`/document/document_type/${router.query.id}/`, {
      ...values,
    ...
    <>
      ...
      <DashboardLayout>
        <LayoutContentWrapper>
          <FormStyles>
            <PageHeader>Update Document type</PageHeader>
            <Box>
