    ...
    const load = async () => {
      ...
      if (!response.data.isEmpty) {
        const dataSource = response.data.map((res, index) => ({
          ...
          info_type: res.info_type?.toUpperCase(),
        }));
        const info_types = response.data.map((res) => ({
          text: res.info_type?.toUpperCase(),
          value: res.info_type.toUpperCase(),
        }));
            ...
            {loading ? (
              ...
              <Space direction='vertical' style={{ width: '100%' }}>
                ...
                <ListingStyles>
                  <Row>
                    <Col span={24}>
                      <Table dataSource={informations} getPopupContainer={(trigger) => trigger.parentNode}>
                        <Column title='Sr.No' key='sr_no' dataIndex='sr_no' />
                        <Column
                          title='Information Name'
                          dataIndex='info_name'
                          key='info_name'
                          sorter={(a, b) => a.info_name.localeCompare(b.info_name)}
                          {...getColumnSearchProps('info_name', 'Information Name')}
                        />
                        <Column
